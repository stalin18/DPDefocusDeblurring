...
class DPDD_PANet2_Num1_Pos2(nn.Module):
    ...
    def forward(self, dp_left, dp_right):
        ...
        # encoder layer 2
        left_output2 = self.conv2(left_pooled_output1)
        left_pooled_output2 = self.pool2(left_output2)

        right_output2 = self.conv2(right_pooled_output1)
        right_pooled_output2 = self.pool2(right_output2)

        # encoder layer 3
        left_output3 = self.conv3(left_pooled_output2)
        left_pooled_output3 = self.pool3(left_output3)

        right_output3 = self.conv3(right_pooled_output2)
        ...
        # encoder layer 5
        left_output5 = self.conv5(left_pooled_output4)
        left_drop_output5 = self.drop5(left_output5)

        right_output5 = self.conv5(right_pooled_output4)
        right_drop_output5 = self.drop5(right_output5)


        # decoder layer 1
        upsampled6_output = self.upsample6_relu(self.upsample6(upsample(torch.cat((left_drop_output5, right_drop_output5), dim=1))))
        merged6 = torch.cat((upsampled6_output, torch.cat((left_output4, right_output4), dim=1)), dim=1)
        ...
        merged7 = torch.cat((upsampled7_output, torch.cat((left_output3, right_output3), dim=1)), dim=1)
        ...
        merged8 = torch.cat((upsampled8_output, torch.cat((left_output2, right_output2), dim=1)), dim=1)
        ...
        merged9 = torch.cat((upsampled9_output, torch.cat((left_output1, right_output1), dim=1)), dim=1)
